...
aov_r <- function(data,
  ...
  between <- if(!is.null(.args$between)) {
    if(is.name(.args$between)) deparse(substitute(between)) else between } else NULL
  within <- if(!is.null(.args$within)) {
    if(is.name(.args$within)) deparse(substitute(within)) else within } else NULL
  response <- if(is.name(.args$response)) deparse(substitute(response)) else response
  id <- if(is.name(.args$id)) deparse(substitute(id)) else id

  result <- list()

  suppressMessages(
    suppressWarnings(model <- afex::aov_ez(
      id = id,
      dv = response,
      data = data,
      between = between,
  ...
  if (!is.null(.args$within)) {
    spher.test <- suppressWarnings(expr = {
      afex::test_sphericity(model)
    })
    sphericity <- if (sphericity == "auto") {
      ...
      if (purrr::is_empty(spher.test) || all(spher.test[, 2] > 0.05)) {
        ...
        ges <- all(summary(model)[["pval.adjustments"]][, c("GG eps", "HF eps")] > 0.75)
        if (ges) {
          "HF"
        } else {
          "GG"
    ...
    } else if (purrr::is_empty(spher.test)) {
      "none"
    } else {
      sphericity
    }
  ...
  efs <- if (es == "eta") {
    effectsize::eta_squared(model, ci = 0.95, generalized = generalized)
  } else if (es == "omega") {
    effectsize::omega_squared(model, ci = 0.95)
  } else {
    stop('You have to choose between "eta" or "omega"')
  }
  ...
  efs[, -1L] <- round(efs[, -1L], 2)
  if (at$correction == "none" || is.null(at$correction)) {
    at$correction <- "Fisher"
  }
  et <- if (markdown) {
    paste0("$\\", es, "^2", if (es == "eta") {
      if (generalized) {
        "_G"
      } else {
        "_p"
  ...
  if (markdown) {
    ...
    for (i in row.names(model)) {
      ...
      result[["full"]][[j]] <- paste0(
        stats <- paste0(
          "*F*~", if (!is.null(within) && any(grepl(within, i))) at$correction else "Fisher",
          "~ (", rt$`num Df`,
          ", ", rt$`den Df`,
          ") = ", rt$F,
          ", *p* ", rt$`Pr(>F)`
